	...
	public PlayWait(PlayClient client) {
		...
		saySomething.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int curIndex = wrUserList.getSelectedIndex();
				if (curIndex >= 0 && curIndex <= wrUserList.getMaxSelectionIndex()){
					System.out.println(wrUserList.getSelectedValue() + ", " + client.nickName);
					if (wrUserList.getSelectedValue().equals(client.nickName))
				...
				if (e.getClickCount() == 2) {
					...
					if (index >= 0) {
						try {
							...
							String[] s = wrRoomList.getSelectedValue().split("]");
							if(s[0].equals("[오셀로 방"))
								p.setCmd(300); // 방 참여
							else if(s[0].equals("[오목 방"))
								p.setCmd(310);
							else
								p.setCmd(333);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (res.length() <= 0 || res == null) {
					try {
						...
						System.out.println("방이름 null처리");
					}
				} else {
					try {
						Protocol p = new Protocol();
						p.setCmd(222); // 오목 방만들기
						p.setMsg("[오목 방]" + res); // 방이름
						oos.writeObject(p);
						oos.flush();
					} catch (NullPointerException e3) {
